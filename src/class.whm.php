...
class WHM
{
	...
	{
	    $d = $this->open('suspendacct?user=' . $username . '&reson=' . $reason);
	    $res = $this->getResult($d);
	...
	public function list_packages()
	{
	    ...
	    foreach ($pkg as $pk) {
	        $out[] = array(
	            'name' => $pk->name,
	            'bandwidth' => ($pk->BWLIMIT == 'unlimited') ? 'unlimited' : $this->get_size($pk->BWLIMIT),
	            'quota' => ($pk->QUOTA == 'unlimited') ? 'unlimited' : $this->get_size($pk->QUOTA),
	            'sql' => $pk->MAXSQL,
	            'sub' => $pk->MAXSUB,
	            'park' => $pk->MAXPARK,
	            'addon' => $pk->MAXADDON,
